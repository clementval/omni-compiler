 ...
 */
/* copyin 021 :
 * copyin 宣言した変数をfor directiveのchunkサイズに指定した場合の動作を確認
 */
...
main ()
{
  ...
  for (prvt=1; prvt<=10; prvt++) {
    #pragma omp parallel for schedule(static, prvt) copyin(prvt)
    for (i=0; i<LOOPNUM; i++) {
  ...
  if (errors == 0) {
    printf ("copyin 021 : SUCCESS\n");
    return 0;
  } else {
    printf ("copyin 021 : FAILED\n");
    return 1;
  }
}
